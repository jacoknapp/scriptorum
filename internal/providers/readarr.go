...
func normalize(i ReadarrInstance) ReadarrInstance {
	...
	if !strings.Contains(i.LookupEndpoint, apiVersionPrefix) { i.LookupEndpoint = apiVersionPrefix + "/book/lookup" }
	if !strings.Contains(i.AddEndpoint, apiVersionPrefix) { i.AddEndpoint = apiVersionPrefix + "/book" }
	if strings.TrimSpace(i.AddMethod) == "" { i.AddMethod = http.MethodPost }
	return i
}
...
func (r *Readarr) PingLookup(ctx context.Context) error {
	...
	resp, err := r.cl.Do(req); if err != nil { return err }
...
func (r *Readarr) AddBook(ctx context.Context, candidate Candidate, opts AddOpts) ([]byte, []byte, error) {
	...
	buf := &bytes.Buffer{}
	if err := tpl.Execute(buf, map[string]any{"Candidate": candidate, "Opts": opts}); err != nil { return nil, nil, err }
...
func (r *Readarr) LookupByTerm(ctx context.Context, term string) ([]LookupBook, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode >= 400 { return nil, errStatus(resp.Status) }
	var arr []LookupBook
...
func hasIdent(b LookupBook, kind, value string) bool {
	...
	for _, id := range b.Identifiers {
		...
		if strings.EqualFold(kt, kind) && strings.ToUpper(vv) == v { return true }
	}
	return false
}
...
func (r *Readarr) SelectCandidate(list []LookupBook, isbn13, isbn10, asin string) (Candidate, bool) {
	...
	if c13 != "" { if cand, ok := pick(func(b LookupBook) bool { return hasIdent(b, "ISBN13", c13) }); ok { return cand, true } }
	if c10 != "" { if cand, ok := pick(func(b LookupBook) bool { return hasIdent(b, "ISBN10", c10) }); ok { return cand, true } }
	if ca  != "" { if cand, ok := pick(func(b LookupBook) bool { return hasIdent(b, "ASIN",  ca) }); ok { return cand, true } }
	return nil, false
}
