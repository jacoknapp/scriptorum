...
func NewAmazonPublic(marketplace string) *AmazonPublic {
	if strings.TrimSpace(marketplace) == "" { marketplace = "www.amazon.com" }
	return &AmazonPublic{
...
func (a *AmazonPublic) GetByASIN(ctx context.Context, asin string) (*PublicBook, error) {
	...
	if asin == "" { return nil, errors.New("asin required") }
	...
	if b.Title == "" { b.Title = strings.TrimSpace(doc.Find("meta[property='og:title']").AttrOr("content", "")) }

	b.Image = doc.Find("meta[property='og:image']").AttrOr("content", "")
	if b.Image == "" { b.Image = doc.Find("#imgBlkFront, #ebooksImgBlkFront, #imgTagWrapperId img").AttrOr("src", "") }
	...
	doc.Find("#bylineInfo span.author a, #bylineInfo a.contributorNameID, .author a.a-link-normal").Each(func(_ int, s *goquery.Selection){
		...
		if name != "" && !contains(b.Authors, name) { b.Authors = append(b.Authors, name) }
	})
	...
	doc.Find("#detailBullets_feature_div li").Each(func(_ int, s *goquery.Selection){
		...
		if strings.EqualFold(label, "ISBN-10") && b.ISBN10 == "" { b.ISBN10 = onlyISBN(val) }
		if strings.EqualFold(label, "ISBN-13") && b.ISBN13 == "" { b.ISBN13 = onlyISBN(val) }
	})

	doc.Find("#productDetailsTable .content ul li").Each(func(_ int, s *goquery.Selection){
		text := strings.TrimSpace(s.Text())
		if strings.Contains(text, "ISBN-10") && b.ISBN10 == "" { b.ISBN10 = onlyISBN(text) }
		if strings.Contains(text, "ISBN-13") && b.ISBN13 == "" { b.ISBN13 = onlyISBN(text) }
	})
...
// SearchBooks scrapes Amazon's public search results (first page) for books by keyword.
func (a *AmazonPublic) SearchBooks(ctx context.Context, q string, limit int) ([]PublicBook, error) {
	if limit <= 0 { limit = 10 }
	...
	doc.Find("div.s-result-item[data-asin]").Each(func(_ int, s *goquery.Selection) {
		...
		if strings.TrimSpace(asin) == "" { return }
		...
		s.Find(".a-row .a-size-base").Each(func(_ int, aSel *goquery.Selection){
			...
			if name != "" && len(authors) < 3 { authors = append(authors, name) }
		})
...
func onlyISBN(s string) string {
	...
	s = strings.ToUpper(s)
	if len(s) >= 13 { return s[:13] }
	if len(s) >= 10 { return s[:10] }
	return s
}

func contains(a []string, v string) bool { for _, e := range a { if e == v { return true } }; return false }
...
func ExtractASINFromInput(s string) string {
	...
	if strings.Contains(ls, "amazon.") {
		...
		for _, p := range parts { if len(p) == 10 { return p } }
	}
