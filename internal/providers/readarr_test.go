...
func TestSelectCandidateIdentifierPriority(t *testing.T) {
	...
	if cand, ok := ra.SelectCandidate(list, "9781234567897", "", ""); !ok || cand["title"].(string) != "B" { t.Fatalf("isbn13 should win: %+v", cand) }
	if cand, ok := ra.SelectCandidate(list, "", "1234567890", ""); !ok || cand["title"].(string) != "A" { t.Fatalf("isbn10 should win") }
	if cand, ok := ra.SelectCandidate(list, "", "", "B012345678"); !ok || cand["title"].(string) != "C" { t.Fatalf("asin should win") }
}
...
func TestReadarrAddBookRoundTrip(t *testing.T) {
	...
	if err != nil || len(res)==0 { t.Fatalf("lookup: %v %v", len(res), err) }
	cand, ok := ra.SelectCandidate(res, "9781111111111", "", "")
	if !ok { t.Fatalf("no cand") }
	pay, resp, err := ra.AddBook(context.Background(), cand, AddOpts{})
	if err != nil { t.Fatalf("add: %v", err) }
	var m map[string]any
	_ = json.Unmarshal(pay, &m)
	if m["title"] != "Book" { t.Fatalf("payload mismatch: %s", string(pay)) }
	if len(resp) == 0 { t.Fatalf("empty resp") }
}
