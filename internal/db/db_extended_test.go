...
func TestDatabaseFileCreation(t *testing.T) {
	...
	// Execute a simple query to ensure database is actually created
	ctx := context.Background()
	err = database.Exec(ctx, "CREATE TABLE IF NOT EXISTS test (id INTEGER)")
	if err != nil {
		t.Fatalf("Failed to create test table: %v", err)
	}
	...
	if _, err := os.Stat(dbPath); os.IsNotExist(err) {
		t.Error("Database file should exist after opening and creating table")
	}
}
